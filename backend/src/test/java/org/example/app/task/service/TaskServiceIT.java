...
class TaskServiceIT {
  ...
  void shouldAllowCreatingANewTaskList() {
    ...
    Response response = given().when().header("Authorization", "Bearer " + token).body("{ \"title\": \"Shopping List\" }").contentType(ContentType.JSON)
        .post("/task/list");
  ...
  void shouldAllowAddingATaskToATaskList() {
    ...
    Response response = given().when().header("Authorization", "Bearer " + token).body("{ \"title\": \"Buy Milk\", \"taskListId\": " + this.taskListId + " }")
  ...
  void shouldAllowRetrievingATaskListWithTaskItems() {
    ...
    given().when().header("Authorization", "Bearer " + token).get("/task/list-with-items/{taskListId}", this.taskListId).then().statusCode(200)
  ...
  void shouldAllowDeletingATaskListCompletely() {
    ...
    given().when().header("Authorization", "Bearer " + token).delete("/task/list/{taskListId}", this.taskListId).then().statusCode(204);
    given().when().header("Authorization", "Bearer " + token).get("/task/list/{taskListId}", this.taskListId).then().statusCode(404);
    given().when().header("Authorization", "Bearer " + token).get("/task/item/{itemId}", this.taskItemId).then().statusCode(404);
