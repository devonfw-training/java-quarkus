...
You might notice, that there are no models available or the container is not even running. Look at the logs from
`docker compose up` - can you spot the issue?

You can provide a model by placing it under `~/root/.ollama/models` on your host, that is then mapped into the
container.

In case the model is still not running anymore, you can start it by
[source, bash]
----
